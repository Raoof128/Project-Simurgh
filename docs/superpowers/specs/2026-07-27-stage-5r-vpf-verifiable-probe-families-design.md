...
N5a forbidden-surrogate suppression is a NO-OP: suppressing changes nothing
    because the suppression machinery never ran                  (§3.4)
...
**Lane A — byte-stable control corpus, CI-gated.** The controls of every **attempted** family, and
their receipts, are a committed corpus generated by a CLI main and proven idempotent — generated
twice into separate paths and compared byte-for-byte, then compared against the committed copy. The
corpus is the attempted set rather than all 165, because §11.5 makes a tranche the expected outcome
and a corpus that must hold 165 controls before anything verifies would make the tranche rule
unshippable. What keeps that honest is `family_result_root`, which is **total over all 55 pairs**
(§10.1): every pair carries a terminal state, so the unattempted are named rather than absent. Every
admissibility condition in §4.1 appears at least once in a **failing** variant, so the tamper matrix
covers the seven conditions rather than only the happy path. Includes a
...
family_result_root               TOTAL over all 55 pairs: each is exactly one of
                                 attempted_admissible | attempted_inadmissible |
                                 not_attempted — so omission is visible by row
control_receipt_root             the control receipts of attempted families,
                                 three per family, incl. restoration proofs
delta_ledger_digest              §6
prior_family_audit_digest        §7.3
vpf_finding_ledger_digest        5R's own findings
```

Signed by a new 5R key. Verifying the inherited 5Q signature needs the 5Q **public** key, which is
committed inside the signed envelope as `signer.public_key_b64` and pinned by
`expected_public_key_digest`; it does **not** need 5Q's private key, and an earlier draft of this
section said it did. A verifier that required a producer's private key would not be a verifier. 5Q's
private key is irrelevant to 5R and its survival is not a 5R dependency.

The 5R key does not sign anything 5Q signed, and the 5Q key signs nothing of 5R's: a successor
signing with its predecessor's key makes the two indistinguishable, which is the property 5G spent a
stage establishing.
