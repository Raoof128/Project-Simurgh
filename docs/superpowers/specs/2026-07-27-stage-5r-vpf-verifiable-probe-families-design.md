...
their receipts, are a committed corpus generated by a CLI main and proven idempotent (`generate twice

- git diff --exit-code`). The corpus is the attempted set rather than all 165, because §11.5 makes a
tranche the expected outcome and a corpus that must hold 165 controls before anything verifies would
make the tranche rule unshippable. What keeps that honest is `family_result_root`, which is **total
  over all 55 pairs** (§10.1): every pair carries a terminal state, so the unattempted are named rather
  than absent. Every admissibility condition in §4.1 appears at least once in a **failing** variant,
