...
- [ ] **Step 1: Write the E2E net** — one file, sections:
  ...
  5. **Privacy scan:** walk every committed byte under `tests/fixtures/llmShield/stage4p/` and `docs/research/llm-shield/evidence/stage-4p/` (excluding `test-keys/`), assert no `-----BEGIN … PRIVATE KEY-----` header, no `observed_evidence_digest` key in the evidence dir OR in any published `signal`/attestation object (MF1 — raw evidence never published; it MAY appear inside test-fixture `input` blocks under `tests/fixtures/.../cpc/`, which are synthetic build scaffolding, so scope the check to evidence-dir bytes + published-signal objects, not raw fixture-tree text), no `http://`/`https://`, no `@`-email pattern; assert public keys only under the `test-keys/` allowlist path or `signer_public_key`/`*_public_key_pem` fields. Pure Node `fs` walk — no `rg`.
