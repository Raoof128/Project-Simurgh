...
1. Test `exitCodes.test.js`: 384–395 unique, disjoint from 364–383; public order is the 11-permutation with 394 before 393 and **395 NOT in it**; RUN*LEVEL all 1. Fail → add block additively (mirror `VTCQ*\*`). Keep the non-hermetic 4h builder reading only its existing two exit-maps.
