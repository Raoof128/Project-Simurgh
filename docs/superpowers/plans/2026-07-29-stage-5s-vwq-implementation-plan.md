...
| 5S-F012 | **An intermittent failure in Stage 4J, ~1 run in 6 of the full suite.** `tests/e2e/llmShield/stage4jFullSmoke.test.js:101` — "E2E: byte-stable golden" — fails with `SyntaxError: Unexpected end of JSON input` from `readJson` at `verify-stage4j-pcta.mjs:16`, called from `loadDfiSubstrate:36` (`pack: readJson(\`${base}-base-pack.json\`)`) inside the regenerated-matrix loop. A JSON file is read empty or truncated. Reproduced twice in thirteen full-suite runs; **never** reproduced by the test alone, including 15 runs on `origin/main`under six-way CPU load, so it needs real full-suite parallelism. Ruled out with evidence: the 4J builder does **not** mutate the committed tree (md5 unchanged,`git status`clean after a run); builder output is complete and parseable across 25 builds under load; no`writeFileSync`anywhere in`tests/`or`tools/`targets the shared 4H fixture paths;`tamperClosure`reads`q0-clean-disconnected-untrusted`and writes only into its own`outputDir`; `.remember/`is gitignored so hook writes are invisible to tests; temp filesystem has 38Gi free, 1% inodes, no leaked directories | **RESOLVED — the defect is a non-atomic write, measured directly.** `writeFile`truncates then fills; between those moments the file exists and is empty. Measured in isolation: writing a 400 KB file 400 times while a reader spun produced **90 truncated reads**; the identical load through write-then-rename produced **0 in 1389**. The 4H builder rewrites the shared committed substrate that`loadDfiSubstrate`reads,`stage4hFullSmoke.test.js`runs it, and`node --test` runs test FILES in parallel processes. Looping the builder against repeated 4J runs reproduced the exact reported error, 1 in 43. That reproduction is mechanistically tied rather than merely correlated — a truncated JSON read REQUIRES a concurrent writer, and machine load cannot truncate a file nobody is writing. Repaired by routing all 15 write sites through an atomic write-then-rename, which fixes every reader of these fixtures including ones not yet written. **A separate three-arm comparison (4/70 shared-tree vs 0/70 temp vs 0/70 idle) is WITHDRAWN as confounded**: the arms ran at different times under different background load, and neither an instrumented rerun (0/70) nor a properly interleaved A/B (0/40 vs 0/40) replicated it. Nothing in this repair rests on it. Original disposition follows — RECORDED against Stage 4J, not 5S —`tests/e2e/llmShield/stage4jFullSmoke.test.js`and`tools/simurgh-attestation/stage4j/`are outside Annex S, and 5S must not edit another stage's tests under its own surface (same boundary as 5S-F007). Reproduction recipe: run`node --test "tests/unit/**/\*.test.js" "tests/e2e/**/\*.test.js"` repeatedly; expect ~1 failure per 6 runs. A load-time source-rewrite probe (`module.registerHooks`) that makes `readJson`name its own path and length is the instrument for the next attempt — **note that patching`fs.readFileSync`does NOT work**, because ESM named imports bind the original function, which left two earlier probes silently inert. Until the mechanism is known, the suite is reported as`1 environment-dependent skip` **and** this known flake, never as "all tests passed" |

| 5S-F013 | **A second intermittent failure, in Stage 4K, distinct from 5S-F012.** `tests/e2e/llmShield/stage4kAllFunctions.test.js:337` — "CLI matches the programmatic API" — fails at line 352, `assert.equal(under.status, 0, under.stderr)`, with `3 !== 0`. Exit **3** is the CLI's catch-all in `verify-stage4k-eba.mjs:165` (`main().catch(...)`), which always writes `stage4k eba: ${e.message}` to stderr first — yet the assertion fell back to its DEFAULT message, so `under.stderr` was empty. A catch-all exit with no diagnostic is the interesting part. Caught once in seven full-suite runs; **not** reproduced by the test alone in 10 runs, with or without an inherited `NODE_OPTIONS` preload. **Correction:** the inference "therefore it needs full-suite parallelism" does not follow. 5S-F014 turned out to be a 1-in-256 randomness defect, and 10 isolated runs have a 4% chance of catching such a thing — so these isolation runs were far too short to distinguish "needs parallelism" from "rare and random". The cause of F013 remains **unknown** | **RESOLVED — the root cause is the SAME defect as 5S-F012, measured directly.** The 4K verifier reads the Stage 4H substrate from a hardcoded relative path (`verify-stage4k-eba.mjs:16`, `const H = "tests/fixtures/llmShield/stage4h/q0-clean-disconnected-untrusted"`), and step 1 reads `${H}-base-pack.json` — the exact file F012's non-atomic write truncated — inside a `try/catch` that converts ANY read or parse error into raw 29 `INTERNAL_ERROR_FAIL_CLOSED` / reason `bundle_unreadable` / typed 3, printing nothing. That is why the two findings looked unrelated: 4J reads the same bytes bare and throws a loud `SyntaxError`, 4K catches and refuses silently. Two measurements, each against the arm it tests. **Sufficiency, deterministic:** truncating that file to 4,000 bytes and running the CLI on the committed `under-budget` bundle reproduces F013's signature exactly — exit 3, stderr 0 bytes, `rawCode 29`, `reason bundle_unreadable`. **Susceptibility, A/B:** a tight-loop reader of that file while the real 4H builder loops, with both arms running CONCURRENTLY so they share identical background load, gave 666 truncated reads in 2,779,311 against the pre-fix `writeFile` builder (652 of them the exact string `Unexpected end of JSON input`) and **0 in 2,773,312** against the write-then-rename builder. The atomic-write repair therefore already retired this finding; no further change is needed. What is NOT claimed: the observed field frequency (~1 in 7 full-suite runs) has not been reconciled arithmetically with the measured truncation rate, and F013 has not been re-observed in the wild since the repair — the mechanism is established, the frequency accounting is not. The earlier disposition follows, retained rather than rewritten because a record that quietly edits its own wrong turns teaches nothing. — the diagnostic-loss defect is fixed; the ROOT CAUSE WAS UNKNOWN at the time of writing and was not claimed.** The finding's own statement that exit 3 is 'the CLI's catch-all' was WRONG — the catch-all always prints `stage4k eba: ${e.message}` first, and no message appeared. Raw codes 1-18, 29 and 39 all map to typed 3 through `stage4CodeForRawCode`, and each of those paths sets `process.exitCode` and prints nothing. So exit 3 with empty stderr is a SILENT REFUSAL, not a crash. Three hypotheses were tested and discarded with evidence: the 4K builder writing a shared tree (all three call sites use a temp `STAGE4K_FIXTURE_OUT`), `process.exit` dropping piped stderr (0 losses in 60 spawns, short and 200 KB), and `CHECKER_NOT_OFFLINE` (raw 28 maps to typed 2, not 3). A trigger-class comparison against the 4H builder was withdrawn as confounded by background load, and did not replicate under an interleaved A/B (0/40 vs 0/40). The repair is instrumental: the test now reports rawCode, typed and reason from the report it had already written and never read, so the NEXT occurrence names itself. Original disposition follows — RECORDED against Stage 4K, not 5S — outside Annex S. Distinct from F012: different stage, different file, different failure mode (a rejected CLI `main()` versus a truncated JSON read), and neither has been shown to share a cause. **That sentence is now known to be wrong on two of its three counts, and is kept because being wrong in a recorded way is the point of the ledger: the file is the SAME file, and the cause is the SAME cause. Only the stage differed. The reading was built on the structural observation that the 4K BUNDLE does not reference a 4H fixture — true, and irrelevant, because the 4K VERIFIER hardcodes one.\*\* The instrument for the next attempt rewrites the assertion at load time to report the whole `spawnSync` result — status, signal, error, stderr, stdout — because the committed assertion discards everything except the status when stderr happens to be empty |
...
| 5S-F017 | **A guard placed after an early return is only exercised once everything before it passes — so the Q1-F006 repair shipped with two v1 readings that no test could reach.** The v2 split moved `entry_count` into `current` and the baseline tag into `baseline`, but `checkProblemGateSet.mjs` kept reading `pin.entry_count` and `pin.baseline_tag` at the top level. A correctly re-pinned v2 census failed with `the pin declares entry_count undefined but carries 23 entries`. The reading sits below the set comparison, which returns first on ANY drift — and a stale census is exactly when a human runs this checker — so the lines only became reachable on the first day the repair worked. The shipped tests covered the v1 fallback and `baseline` immutability but never the one workflow the repair exists to enable: re-pin `current`, then run the checker. Seeded-red on the parts that WERE tested gave a false sense of coverage, because the untested part was invisible rather than failing | **REPAIRED on Stage 5Q's own branch (PR #129), not in 5S — the file is `path_not_in_surface` for Annex S.** Both readings now fall back to the v1 shape. The test added with the fix is the deliverable: it builds a synthetic repository with one manually-enumerated completeness gate, pins it correctly as v2, runs the REAL driver through `spawnSync`, and demands exit 0 plus a success line naming the baseline tag — seeded red against the original defect, it reproduces the failure verbatim. Two false starts building the fixture are themselves the warning: the first synthetic step was not a completeness-asserting gate at all, so the census correctly ignored it and the test would have passed against an empty problem set. **The generalisable rule: when a refactor moves a field, the guards BELOW an early return are the ones no existing test reaches, and the check that finds them is an end-to-end run of the workflow the refactor was for** |

| 5S-F018 | **Two Stage 5S proof assertions hard-required a `lean` binary that `scripts/check.sh`'s own CI job installs only AFTERWARD — green on every developer machine, red in CI, reproduced by neither.** `tests/unit/llmShield/stage5s/theoremSet.test.js` shells out to `lean` (the type-check) and to `scripts/check-lean-proofs.mjs` (the repo-wide gate). In `.github/workflows/stage-1-checks.yml` the step order is `Run Stage 1 check suite → ./scripts/check.sh` at line 114, then `Install Lean (elan)` at line 121 — so the toolchain arrives seven lines and one step too late, and the gate fails closed with `lean_gate_missing_lean_binary`. CI reported `# tests 5053 · pass 5050 · fail 2 · skipped 1` in all four jobs; every local run reported `5053 · 5053 · 0 · 0`, including one with `SIMURGH_SKIP_DOTENV=1` and CI's entire env block, because this machine has `lean` at `/opt/homebrew/bin`. **`check.sh` truncates each step's log to its last 40 lines, and the failures had scrolled past — CI could say two tests failed but not which**, and the uploaded log artifact was empty. Identified by reading the workflow's step order rather than by reproducing it | **REPAIRED in 5S.** The two binary-dependent assertions now SKIP with a named reason instead of failing; the source-level escape-hatch scan is untouched and unconditional, which matters because it — not the type-checker — is what catches `sorry` (`lean` exits 0 on a sorry-closed theorem). A skip is only honest if the check runs somewhere, so a permanently-executing test asserts the workflow installs elan, that a step RUNS the repo-wide gate, and that the install comes FIRST. Seeded red in both directions: deleting the elan step and replacing the gate step each redden it. **Its first draft was itself wrong** — it compared raw `indexOf`, and `check-lean-proofs.mjs` also appears in the workflow's `paths:` trigger far above the steps, so a correct workflow went red. A path filter naming a script is not a step running it: the quote/mention distinction, one more time, this time inside a guard written to prevent a vacuous green. The skip is declared in the closeout, which now states that the skip count is environment-dependent — one here, three on a runner without a toolchain |

| 5S-F019 | **The committed Stage 4H exit-map on `main` is stale by eleven codes, so running the 4H builder on a clean checkout dirties the tree.** `docs/research/llm-shield/evidence/stage-4h/exit-map.json` and `tests/fixtures/llmShield/stage4h/expected-results/exit-map.json` stop at 463; the builder adds 464-474, which are **Stage 5P's** codes. 5P shipped without rippling the 4H exit map. Verified as pre-existing rather than assumed: the builder was run on a pristine `origin/main` checkout with no local changes, and produced the same two modifications | **NOT repaired in isolation — Stage 5S's own Annex M ripple already fixes it.** 5S adds codes 475-512 and regenerates both projections under rows 5S-M001/M002, so its exit map runs to 512 and includes 464-474; the builder leaves the 5S tree clean. The lesson is the one worth carrying: an additive raw-band ripple is a stage obligation, and 5P's omission stayed invisible for two stages because nothing compares the committed exit map against what the builder produces. A gate that runs the builder and refuses a dirty tree would have caught it the day it landed |
| 5S-F020 | **Stage 5Q's CI gate is scoped by TRIGGER but unscoped in EVALUATION, so any pull request touching a 5Q file plus anything else is refused.** The `paths:` filter correctly limits the workflow to 5Q-owned paths — that part is Q1-F005 working. But once triggered, `checkWriteSurface.mjs --range origin/main...HEAD` judges the WHOLE diff against 5Q's exhaustive §6.1 surface, so unrelated files in the same PR are reported `path_not_in_maintenance_surface`. Observed on a five-fix PR: fourteen unrelated paths refused, including `scripts/check.sh` and four other stages' tests | RECORDED against Stage 5Q, not repaired here. Worked around by splitting the 5Q change into its own pull request (#132), which is correct practice regardless and keeps review coherent. The underlying tension is real and belongs to whoever next maintains that gate: a stage's write surface is a statement about what THAT STAGE may change, and after the stage ships, a post-hoc PR touching one of its files is not the same act. The repair is probably to intersect the judged change set with the trigger paths, so the gate measures its own stage's discipline rather than whatever else happened to be in the branch — which is the Q1-F005 sentence, one level in |
| 5S-F021 | **The repo-wide F014 guard this stage shipped could not tell a DESCRIPTION of the banned pattern from a USE of it.** It scanned raw source text, so the day 5S's branch met the repaired `main` it reddened against four passages of 5S's own documentation: three comments in `tests/e2e/llmShield/stage5s/tamper.test.js` and `tools/simurgh-attestation/stage5s/fixtures/tamperCensus.mjs` that write the defect down because this is the stage that FOUND it, plus one executable line — `assert.equal("00" + alreadyZero.slice(2), alreadyZero)` — kept deliberately so a future tidy-up cannot delete the reason `flipFirst` exists. The guard's own file was exempted wholesale, which hid the flaw from its author: the one file guaranteed to describe the pattern was the one file never scanned. **This is the FOURTH quote/mention failure in this stage**, after the closeout claim gate, the Lean toolchain-order guard, and the census parser — a personal failure mode, not four coincidences | **RESOLVED on its own branch (PR #133).** Two rules, both narrow, neither able to hide a real defect: a match inside a comment is not a use (comments do not execute), and a match on a line that ASSERTS the result equals the original is the defect being demonstrated rather than used — a real tamper asserting it changed nothing would be asserting its own uselessness. The line-comment scan is quote-aware, so a `"https://"` inside a string cannot blind the guard, which is the false-NEGATIVE direction and the only one that matters. Seeded red in context rather than in theory: both real 5S files scan to zero offenders, and a genuine defect appended to each is still caught. Near-misses that must still redden are asserted — code on the line after a comment, a string containing `//`, and an `assert.equal` whose tampered value is compared against something else |
