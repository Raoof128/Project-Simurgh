...
run_step 30 node tools/simurgh-attestation/stage4k/verify-stage4k-eba.mjs \
  --bundle "$FIX/bundles/under-budget" --pinned-pubkey "$FIX/eba-signer.pub"
if node tools/simurgh-attestation/stage4k/verify-stage4k-eba.mjs \
  --bundle "$FIX/bundles/over-budget" --pinned-pubkey "$FIX/eba-signer.pub" >/dev/null 2>&1; then
  echo "reproduce: over-budget bundle passed — Q8 is theatre" >&2
...
2. Over-budget bundle → raw `30` → run-level `1`; deletion → fail-closed; unsigned tamper → `22`; bad/unpinned signature → `25`; unknown class → `29` — all pinned by tests.
3. Two consecutive temp builds are byte-identical on ledger/attestation/matrix AND match the committed fixtures byte-for-byte.
4. `npm test` fully green; `npm run format:check` clean; zero `src/llmShield` diffs; both 4H exit maps committed with the `30` row and nothing else changed.
5. Closeout + README carry all §0.5 non-claims verbatim; overclaim guard green; raw `39` reservation named.
