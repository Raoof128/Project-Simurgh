...
- `tools/simurgh-attestation/stage4k/emit-stage4k-evidence.mjs` — evidence emitter with divergence refusal; `STAGE4K_FIXTURE_ROOT`/`STAGE4K_EVIDENCE_OUT` env overrides (test-only) added in K7.
...
- `tests/e2e/llmShield/stage4kAllFunctions.test.js` — K7 all-functions integration net (full public surface composed end-to-end).
...
### Task K7: Full E2E all-functions integration test

**Goal:** One final "nothing is theatre" net across the whole Stage 4K surface — prove every exported function composes through the real pipeline (constants → ledger → gate → attestation → manifest → fixture builder → `runEbaCore` → CLI → evidence emitter → reproduce script → exit wrapper), not only through isolated unit tests. Mandatory before release/tag.

**Files:**

- Create: `tests/e2e/llmShield/stage4kAllFunctions.test.js`
- Modify: `tools/simurgh-attestation/stage4k/emit-stage4k-evidence.mjs` (env-overridable paths, defaults unchanged)

**Interfaces:**

- Consumes: every K1–K6 export by exact name — `SIGNAL_CLASS_WEIGHTS` (constants), `EbaSchemaError`/`buildLedger`/`consumerIdDigest`/`ledgerDigest` (extractionLedger), `checkBudgets` (extractionBudgetGate), `buildAttestation`/`buildEbaManifest`/`verifyEbaManifest` (ebaManifest), `runEbaCore` (verify-stage4k-eba), `EBA_RAW_CODES`/`PCTA_RAW_CODES`/`stage4CodeForRawCode` (stage4h/exitCodes), `verifyEvidencePack` (stage4d/verifyPack), `diagnose` (stage4h/dfiCertificate), the committed fixture tree, the emitter CLI, the reproduce script.
- Produces: nothing new — this task only proves composition. The emitter gains `STAGE4K_FIXTURE_ROOT` / `STAGE4K_EVIDENCE_OUT` env overrides (test-only; real runs unchanged).

**Safety note (no recursion):** the reproduce script runs `tests/unit/llmShield/stage4k/` and `stage4kFullSmoke.test.js` — it never invokes `stage4kAllFunctions.test.js`, so this file may safely run the reproduce script (test group 8). It is the heaviest test in the suite (~1 min).

- [ ] **Step 1: Make the emitter paths env-overridable**

In `tools/simurgh-attestation/stage4k/emit-stage4k-evidence.mjs`, replace:
...
Then re-run the emitter without env vars and confirm the committed evidence is byte-identical (defaults still route to the committed paths):

```bash
PATH=/opt/homebrew/bin:$PATH node tools/simurgh-attestation/stage4k/emit-stage4k-evidence.mjs
git diff --exit-code -- docs/research/llm-shield/evidence/stage-4k && echo EVIDENCE_UNCHANGED
```

Expected: `stage4k evidence written to ...` then `EVIDENCE_UNCHANGED`.

- [ ] **Step 2: Write the all-functions E2E test**
...
- [ ] **Step 3: Run the all-functions test**

Run: `PATH=/opt/homebrew/bin:$PATH node --test tests/e2e/llmShield/stage4kAllFunctions.test.js`
Expected: PASS — 14 tests, 13 passing + 1 skipped on this machine (the node-guard test skips unless `/usr/bin/node` is a pre-26 binary). The reproduce-script test takes ~1 min; everything else is seconds.

- [ ] **Step 4: Run the full suite + format**

```bash
PATH=/opt/homebrew/bin:$PATH npm test
PATH=/opt/homebrew/bin:$PATH npm run format:check
```

Expected: fully green / `All matched files use Prettier code style!`. If `format:check` is red, run `npx prettier --write tests/e2e/llmShield/stage4kAllFunctions.test.js tools/simurgh-attestation/stage4k/emit-stage4k-evidence.mjs` and re-check.

- [ ] **Step 5: Commit**

```bash
git add tests/e2e/llmShield/stage4kAllFunctions.test.js tools/simurgh-attestation/stage4k/emit-stage4k-evidence.mjs
git commit -m "test(llm-shield): add stage 4k all-functions e2e integration net"
```
...
7. `PATH=/opt/homebrew/bin:$PATH node --test tests/e2e/llmShield/stage4kAllFunctions.test.js` passes (K7), covering every exported Stage 4K function plus CLI, evidence-emitter refusal, and reproduce-script paths. K7 is MANDATORY before release/tag (K1–K6 may merge first if smaller PRs are needed, but no tag without K7 green).
