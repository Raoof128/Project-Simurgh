...
- [ ] **Step 1: Write the failing test** — build a helper `mkReceipt(parentDigest, name)` returning a minimal unsigned receipt object `{schema, epoch: "ep1", run_id: "run1", window_id: "w1", root_receipt_digest, parent_receipt_digest, delegator_key_digest, delegatee_key_digest, scope: ["mail.read"], budget_allocated: 6, spine_refs: {custody_4p: null, consent_4o: null, friction_4q: null}}`; cover:
  ...
  - disconnected cycle island: one valid rooted tree PLUS a two-receipt A↔B island naming each other as parents (root check passes; 103 passes since digests resolve; not split-brain) → global cycle detection fires BEFORE reachability → 104 (the island shape avoids colliding with 113 — a "rooted cycle" in a parent-pointer tree usually degenerates into split-brain first)
...
- Rules: observed children are grouped by **`(parent_receipt_digest, window_id)`** (hop receipts carry `window_id` — spec §4 patch); duplicate declared digests → `{raw: 100, reason: "duplicate_declared_child_digests"}`; a node/window with NO commitment → 106 (`declared` treated as absent, count mismatch); count mismatch → 106; equal count but different set (or set-root mismatch) → 107. Zero-fan-out leaves MUST carry an explicit empty commitment (§3.2) — a missing leaf commitment is 106. Test MUST include a two-window node: same parent delegating in `w1` and `w2` with one commitment per window → 0; child minted in `w2` but committed under `w1` only → 106.
...
- Produces:
  ...
  - `receiptBuilder.mjs`: `keyDigest(publicKeyPem) -> "sha256:..."`, `buildHopReceipt({...spec §3.1 fields minus signatures})`, `dualSign(receipt, delegatorPrivKey, delegateePrivKey) -> signed receipt` (each signature = Ed25519 over `canonicalJson` of the receipt WITHOUT both signature fields, hex), `verifyDualSignature(receipt, delegatorPubPem, delegateePubPem) -> {ok} | {ok: false, missing: bool}` (missing field vs empty/invalid — spec §11 semantics), `signFanout(commitment, delegatorPrivKey)`, `signCrossing(artifact, actorPrivKey)`, `assembleChainBundle({epoch, runId, treeReceipts, detachedReceipts, fanouts, crossings, publicKeyIndex, spineIndex}) -> bundle` (spec §3.4 structure) computing `bundle_merkle_root = bundleRoot([...tree receipt digests, ...detached receipt digests, ...fanout digests, ...crossing digests])` in that concatenation order (each via recordDigest with its own DOMAINS.\* domain), embedding `public_key_index` (every key digest referenced by any receipt/crossing MUST resolve to PEM material in it — builder throws if not), `spine_index`, `VDCC_NON_CLAIMS`, `VDCC_KNOWN_LIMITATIONS`, `VDCC_RAILS`. Detached receipts are SEALED under the root — adding/removing one breaks 117.
...
- Produces:
  - `evaluateChain(bundle, opts = {}) -> {raw, reason, detail}` — key material defaults to `bundle.public_key_index` (missing/malformed index → 100 `public_key_index_missing_or_malformed`; a referenced key digest absent from the index → 101 `referenced_public_key_unverifiable`); `opts.publicKeyIndex` overrides for audit-tier cross-checks. Runs EXACTLY the frozen check order: schema shape of bundle (spec §3.4 four-array structure)/receipts/fanouts/crossings incl. missing signature fields + duplicate declared digests (100) → every dual signature + fanout + crossing + detached-receipt signature verifies against the index (101) → tree invariants via treeCore over `tree_receipts` ONLY (102/103/113/104/105) → fanoutCore grouped by `(parent_receipt_digest, window_id)` (106/107) → **BINDING-DEFERRAL RULE (spec §11): scope and flux phases evaluate ONLY crossings whose `bound_receipt_digest` resolves to a verified tree node; receiptless/unknown/detached-bound crossings are deferred UNTOUCHED to the binding phase** → scope: per-edge `scopeLeq(child, parent)` and per-resolved-crossing `scopeLeq(requested_scope, pathScope(bound node path))` (108) → fluxCore over resolved crossings (110 then 109) → binding phase: `bound_receipt_digest` empty/missing/unknown (112) → digest exists as a validly signed receipt in `bundle.detached_receipts` but absent from the verified tree and every committed child set (111) → replay: every receipt/crossing `epoch === bundle.epoch` (114) → every receipt `run_id === bundle.run_id && root_receipt_digest === (concrete root digest or ROOT_SENTINEL for root)` (115) → `spine_refs` digests, when non-null, must appear in `bundle.spine_index` (116) → recompute `bundle_merkle_root` over all four sealed arrays (117) → `{raw: 0, reason: "green"}`.
...
| orphan crossing              | crossing bound to receipt in `detached_receipts`                                                                                                                          | 111         |
...
| dropped after seal           | append one extra detached receipt AFTER computing `bundle_merkle_root` (tree, fanout, scope, flux, binding, replay all stay green — only the sealed artifact set drifted) | 117         |
...
- Produces: `python3 vdcc_kernel.py verify <corpus-index.json>` — standalone stdlib reimplementation. **Python parity intentionally excludes Ed25519 verification because zero new dependencies are allowed (stdlib has no Ed25519). It verifies the schema-normalised, post-signature mathematical layers only: tree, fan-out, scope, flux, replay, Merkle** (precedent: `tools/simurgh-attestation/stage4r/python/pccc_kernel.py`). Signature fixtures (the missing-field 100 variant and all 101s) remain JS-only. Prints one canonical JSON line per fixture: `{"name", "raw", "reason"}` with `sort_keys=True, separators=(",", ":")`.
