...
| 3   | `not_grey_market_investigation`                  | `node/build-stage4p-fixtures.mjs` (Lane C section) uses fully synthetic names and falls back to `public_report_custody_data_insufficient` rather than reconstructing an incident; fixture at `tests/fixtures/llmShield/stage4p/lane-c/public-report-motivated`.                            |
