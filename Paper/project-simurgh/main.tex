...
Results: 19 of the 20 sessions scored Safe (risk score $\leq 19$) with no Warning verdicts, and no session produced a Critical or Warning verdict from behavioural signals alone. The single non-Safe session (S11, score~=~85) is a deliberate boundary scenario driven entirely by the affinity channel: a macOS system-tray utility window with \texttt{SharingType.none} set by the OS itself (e.g., a screen-sharing or accessibility helper). This is a known limitation of the affinity override (H02): the override fires on \textit{any} excluded window, including legitimate system utilities. Excluding this affinity case, the false-positive rate for behavioural signals alone is \textbf{0/19 = 0\%} across the clean sessions; the affinity channel has a non-zero false-positive rate dependent on the mix of system utilities on the student's machine. Institutions deploying Simurgh should conduct platform-specific baselining before production use.
