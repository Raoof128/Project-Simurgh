...
# Stage 5F VMP — Lane C capture, member: Llama Guard 3 1B (smaller sibling of LG4 12B; runs on CPU/M2). NON-CI.
# Motto: AnthropicSafe First, then ReviewerSafe. Classifies the SHARED committed corpus and
...
def capture(corpus_path, out_path):
    ...
    model = AutoModelForCausalLM.from_pretrained(MODEL_ID, revision=HF_REVISION, torch_dtype=torch.float16).to(device).eval()
    ...
    for case in corpus["cases"]:
        ...
        enc = tok.apply_chat_template(messages, return_tensors="pt").to(device)
        with torch.no_grad():
            out = model.generate(enc, max_new_tokens=12, do_sample=False)
        raw = tok.decode(out[0][enc.shape[-1]:], skip_special_tokens=True)
        print(f"  [{case['case_id']}] {raw.strip()[:20]!r}", flush=True)
        label = parse_verdict(raw)
        cell = {"case_id": case["case_id"], "member_id": "llama_guard_3_1b", "detector_input_digest": sha256_str(text)}
        if label is None:
        ...
        cells.append(cell)
    fragment = {"member_id": "llama_guard_3_1b", "model_id": MODEL_ID, "hf_revision": HF_REVISION, "runtime": {"device": device, "dtype": "float16"}, "cells": cells}
