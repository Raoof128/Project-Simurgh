...
const UNTRUSTED_LABELS = new Set(["untrusted", "untrusted_web", "untrusted_tool", "provider_output"]);
...
export function deriveIntegritySummary(taintInputs) {
  ...
  return [...labels].some((label) => UNTRUSTED_LABELS.has(label)) ? "contains_untrusted" : "trusted_only";
}

export function deriveUntrustedReachedAuthority(taintInputs) {
  return Boolean(taintInputs.authority_sink) && deriveIntegritySummary(taintInputs) === "contains_untrusted";
}
...
export function decide(policyBundle, decisionInput, material, sinkRegistry) {
  ...
  if (features.sink_id === "authority_escalation" && policyBundle.modes[mode].block_authority_escalation) {
  ...
  if (integrity === "contains_untrusted") {
    ...
    if (features.sink_id === "egress" && policyBundle.modes[mode].block_untrusted_to_external_egress) {
