...
- **Deviation:** A role-play-framed input that *also* contains a literal exfil phrase hard-`blocks` (safer) rather than warns; the warning tier is for role-play framing without a literal denylist hit. Split into two firewall tests covering both paths.
