...
- **Scope:** Add a deterministic, key-free measurement stage that proves a Fable-5-style failure chain is contained *after input filtering fails*. Pure evidence stage — no `src/llmShield/**` guard-logic changes (enforced by a dedicated policy-drift guard).
- **Summary:** A deterministic 180-case corpus (5 malicious families × {24 input-miss + 6 direct} + 30 benign hard-negatives) is driven through the real Simurgh boundary functions in pipeline order (`classifyPrompt → guardContexts → gateToolRequest → scanOutput`), so the observed containment boundary is measured, not asserted. H1 is enforced as a fixture-validity gate: each of the 120 input-miss cases must pass the input firewall and be contained by its *intended* downstream boundary (`case_expectation_mismatches=0`); each of the 30 direct cases must be blocked at the input firewall. Results: input-miss `120/120` passed + downstream-contained, input-firewall containment `0/120`, direct blocked `30/30`, targeted ASR `0/150`, benign `30/30`, context-authority escalation `0`, unsafe tool/export `0`, receipt/audit `180/180`, generated-evidence leakage `0`, policy-drift `0`. The Fable 5 incident is used only as a public reference event (payload-redacted); no jailbreak transcript is committed and no immunity is claimed. Stage 3M (remediation) not triggered.
