...
- **Scope:** New stage on branch `stage-5i-vpc`. Additive raw codes **316–331** in the global `stage4h/exitCodes.mjs`; a new `tools/simurgh-attestation/stage5i/` verifier; new tests + evidence + Lean. No prior verifier semantics, evidence values, model scores, or attestation codes changed — additive only.
- **Summary:** Turns RSP v3.4's split-external-review condition into an offline-recomputable relation: grant-bounded coverage **equality** `⋃C(r)=S` with **computed** reviewer+host independence (5G rung lattice re-instantiated), a **no-silent-filter census** (every supplied member qualifies or fails closed), and externally-anchored non-affiliation. Laws: No Section Left Unreviewed (327), No Phantom Review (323), No Self-Vouched Reviewer (325/326). Headline invention: the **Adequacy Gate** (`VPC_ADEQUACY_CLAIMED`, 328) — a coverage verifier structurally unable to certify review adequacy (fails closed even at full coverage; `noForbiddenAdequacyAssertion`, honestly bounded-vocabulary). Two zero-code projections: Coverage Depth Census + Typed Coverage State. Two roots break the challenge↔root cycle. Pure `vpcCore` owns the frozen order; crypto arrives via `facts` (adapter). Three lanes: A byte-stable committed pack (raw 0 public+audit + byte-stable), B deterministic multi-process ceremony, C real Opus 4.6 public structure — **real independent-party ceremony EXECUTED** (droplet team, own keys distinct from ours, 37 sections, our verifier accepts raw 0; campaign `pending → completed`, evidence_root `sha256:b72ca365…`, fail-closed gate now verifies the pack). JS↔Python↔browser parity byte-identical. 9 Lean theorems (zero sorry).
- **Files changed:** `tools/simurgh-attestation/stage4h/exitCodes.mjs` (codes 316–331), `tools/simurgh-attestation/stage5i/**` (constants, core/\*, node/\*, laneb/\*, lanec/\*, browser/\*, python/\*), `tests/unit/llmShield/stage5i/**`, `tests/e2e/llmShield/stage5i/k7AllFunctions.test.js`, `tests/fixtures/llmShield/stage5i/test-keys/**`, `proofs/stage5i/**`, both `exit-map.json` goldens + `tests/unit/llmShield/stage4h/exitWrapper.test.js` (additive-code ripple), `scripts/reproduce-llm-shield-stage5i.sh`, `scripts/security-audit-llm-shield-stage3{m,o}.sh` (key allowlist), `.prettierignore`, `docs/research/llm-shield/{evidence/stage-5i,STAGE_5I_CLOSEOUT.md}`, `README.md`, `AGENT.md`, `CHANGELOG.md`, spec + plan under `docs/superpowers/`.
- **Verification:** 35 stage5i unit/e2e green incl. the K7 all-functions net (every raw 316–331 reachable + evidence lock); `reproduce-llm-shield-stage5i.sh` **ALL PASS** (Node 26 — Lane A verify+byte-stable, Lane B ceremony, Lane C pending gate, committed==fresh); prior `reproduce-llm-shield-stage5h.sh` still PASS (sealed history); full repo unit suite green except the pre-existing Stage-2 `securityHardening`/`ANTHROPIC_API_KEY` baseline (references no VPC code); `proofs/stage5i` `lean PanelCoverage.lean` compiles zero sorry; JS↔Python↔browser byte-identical digests on the committed pack.
- **Follow-ups:** VRC (rating contest) then VUC (universe commitment) complete the External Accountability arc; VTC (temporal coverage) penciled. Nearest lever: execute the real Lane-C ceremony with the droplet team (flip `pending → completed`).

### 2026-07-10 (Australia/Sydney) — Reproduce-script fail-open sweep (stages 4Y–5D)

**Raouf:**

- **Scope:** Correctness repair to six per-stage `scripts/reproduce-*.sh` runners after the Stage 5E clean-room finding was traced to a shared pattern. No verifier semantics, evidence values, model scores, capture claims, attestation codes, or production runtime code changed — reproduce runners only.
- **Summary:** The independent Stage 5E finding (a gate written `cmd >/dev/null && echo "… OK"` under `set -euo pipefail` fails **open** — `set -e` does not abort when the failing command is the non-final element of an `&&` list, so a failed unit/K7/parity/Lean gate skips its "OK" line but the script sails on to `ALL PASS` and exits 0) was **not unique to 5E**. The same pattern was present in `reproduce-llm-shield-stage{4y,4z,5a,5b,5c,5d}.sh`. Split every affected gate into a standalone command followed by its echo so a failure terminates before the terminal `PASS`/`ALL PASS`. The 5C Lean gate additionally chained `lean … && ! grep sorry && echo` (fail-open on **both** a Lean error and a found `sorry`) — replaced the `! grep` with an explicit `if grep … then exit 1` guard so a stray `sorry` now fails closed. The `git diff --quiet … && echo … || { … exit 1; }` byte-stability lines were left unchanged (they already fail closed via the `|| exit 1` fallback).
...
- **Verification:** Empirically confirmed the pattern fails open (minimal `false && echo OK; echo "ALL PASS"` under `set -e` exits 0). Post-fix: no `&& echo …OK/PASS` fail-open remains in any reproduce script; `bash -n` passes on all six; all six run `PASS`/`ALL PASS` normally under Node 26 (v26.5.0); injecting an exit-73 `node` shim into the 5D unit gate now yields exit 73 with **no** `unit OK` and **no** `ALL PASS` (previously it would have printed `ALL PASS` and exited 0). Note: CI gates each stage with `node --test` directly (not through the reproduce wrapper), so no stage ever shipped a genuinely failing gate — this closes a false-`ALL PASS` exposure for third parties running the reproduce scripts.
- **Follow-ups:** Consider a lint/CI guard rejecting `&& echo` around gate lines in future reproduce scripts; the Stage 5E `conformancePack.test.js` fail-closed regression is the template for a generalized check.

### 2026-07-10 (Australia/Sydney) — Stage 5E droplet evidence log annotation
...
### 2026-07-10 (Australia/Sydney) — Stage 5E conformance pack fail-closed repair
...
- **Scope:** Add a deterministic, key-free measurement stage that proves a Fable-5-style failure chain is contained _after input filtering fails_. Pure evidence stage — no `src/llmShield/**` guard-logic changes (enforced by a dedicated policy-drift guard).
- **Summary:** A deterministic 180-case corpus (5 malicious families × {24 input-miss + 6 direct} + 30 benign hard-negatives) is driven through the real Simurgh boundary functions in pipeline order (`classifyPrompt → guardContexts → gateToolRequest → scanOutput`), so the observed containment boundary is measured, not asserted. H1 is enforced as a fixture-validity gate: each of the 120 input-miss cases must pass the input firewall and be contained by its _intended_ downstream boundary (`case_expectation_mismatches=0`); each of the 30 direct cases must be blocked at the input firewall. Results: input-miss `120/120` passed + downstream-contained, input-firewall containment `0/120`, direct blocked `30/30`, targeted ASR `0/150`, benign `30/30`, context-authority escalation `0`, unsafe tool/export `0`, receipt/audit `180/180`, generated-evidence leakage `0`, policy-drift `0`. The Fable 5 incident is used only as a public reference event (payload-redacted); no jailbreak transcript is committed and no immunity is claimed. Stage 3M (remediation) not triggered.
- **Files changed:** `tests/e2e/llm_shield_stage3l_fable5_reference_{lib,runner}.mjs`, `tests/unit/llmShield/stage3lFable5ReferenceLib.test.js`, `scripts/{smoke,security-audit,privacy-audit,consistency-audit,policy-drift-guard}-llm-shield-stage3l.*`, `docs/research/llm-shield/{LLM_SHIELD_STAGE_3L_FABLE5_REFERENCE_CONTAINMENT,STAGE_3L_THREAT_MODEL,STAGE_3L_VALIDATION_MATRIX,STAGE_3L_REVIEWER_CHECKLIST,STAGE_3L_CLOSEOUT}.md`, `docs/research/llm-shield/evidence/stage-3l/**`, `docs/superpowers/{specs,plans}/*stage-3l*`, `scripts/check.sh`, `README.md`, `AGENT.md`, `CHANGELOG.md`.
- **Verification:** `node --test` unit suite passed (11/11); `scripts/smoke-llm-shield-stage3l.sh` passed (runner + policy-drift + privacy + consistency + security audits); wired into `scripts/check.sh`. Tag target `v1.5.0-stage-3l-fable5-reference-containment`.
...
- **Verified:** `npm test` 589; 3E-live smoke (disabled/missing-key/context-rejected/rate-limit PASS, optional SKIP, fixtures 40/40); security + privacy audits PASS; mock/recorded + 3B no drift; prettier clean; `npm audit` 0 vulns. Tag `v0.7.1-stage-3e-live-anthropic-adapter` after merge.
...
- **Scope:** Stage 3D — reposition the LLM Shield from a jailbreak detector to a jailbreak-_consequence_ container (context provenance / tool gate / output firewall + per-session run risk accumulator), verified against the deterministic mock provider. No live provider, no real tools, no network.
- **Closeout:** all seven phases landed. New modules: `contextCanonicalise`, `contextProvenanceGuard`, `toolPolicy`, `toolInvocationGate`, `outputLeakageFirewall`, `runRiskAccumulator`, `stage3dReceipt`, `stage3dMockScenarios` (each unit-tested). Five e2e smokes + a direct-import fixture runner; 60-case corpus (10/category) all passing; three `*-stage3d` gate scripts; five reviewer docs; receipt samples + captured gate outputs under `evidence/stage-3d/`. Verified: `npm test` 520 pass; 3A/3D smokes; 3B bench no drift; security 7/7 + 9/9; both privacy audits PASS; `npm audit` 0 vulns; prettier clean. Tag `v0.6.0-stage-3d-llm-containment`. Risk thresholds locked (0–2/3–5/6+); point weights tunable. Runner deviation: added a `turns[]` multi-turn fixture format and a fixtures-only `tool_request` override so the corpus exercises true cross-run accumulation and the full tool-policy table (neither reachable over HTTP).
- **Phase-1 foundation detail:** Added an **additive** route gate (`isStage3DRun`): a request carrying `contexts`/`tool_mode`/`scenario`/`stage3d:true` is routed to a new `handleStage3dRun`; plain `{ input }` keeps the byte-for-byte 3A/3B/3C path so the frozen benchmark and `simurgh.llm_safety_receipt.v1`/schema `3C` receipt do not drift. New `stage3dReceipt.js` builds a `schema_version "3D"` metadata-only receipt (reuses the `v1` type and `hashReceipt`; `safetyReceipt.js` untouched). New Stage 3D audit events + ordered `recordStage3dRun`/`recordStage3dReceiptExported`. New committed `stage3dMockScenarios.js` allowlist — the live route maps a bounded `scenario` enum to canned outputs (synthetic markers only); raw `mock_provider_output` is rejected over HTTP (`mock_provider_output_http_rejected`); unknown scenario → `scenario_not_allowed`.
- **Deviation from plan:** the plan flagged the _security-audit script_ contexts assertion, but two more places encoded the retired alpha behavior — `tests/unit/llmShield/router.test.js` ("contexts rejected fail-closed") and the Stage 3A smoke fixture `evidence/stage-3a/fixtures/contexts-rejection/llm-alpha-contexts-001.json`. Updated the unit test to assert 3D activation and **retired** the alpha fixture (the `contexts_not_supported_alpha` limitation it documented no longer holds; Stage 3D's context-provenance corpus supersedes it). Updated the stage-3a README accordingly.
