...
import { verdictAt, sourceDigest } from "../../../../tools/simurgh-attestation/stage5d/core/gateRegistry.mjs";
import { classifyDurability, CLOSURE_RULE_KINDS } from "../../../../tools/simurgh-attestation/stage5d/core/durability.mjs";
import { cornerOutcomes, trilemmaHolds } from "../../../../tools/simurgh-attestation/stage5d/core/trilemma.mjs";
...
test("tamper matrix: every code 240–254 fires at its owning tier, frozen first-failure order", () => {
  ...
  assert.equal(audit(signed((c) => ((c.rungs[0].evasions[0].evasion_digest = "sha256:0"), c))), 244);
  assert.equal(audit(signed((c) => ((c.rungs[0].evasions[0].watcher_verdict_at_target = true), c))), 245);
  ...
  assert.equal(
    audit(signed((c) => ((c.trilemma_corners[1].closes_confusables = true), (c.trilemma_corners[1].diacritic_overblock = false), c))),
    249
  );
  assert.equal(audit(signed((c) => ((c.byo_target = { schema: "wrong" }), c))), 250);
  assert.equal(
    audit(signed((c) => ((c.attester_provenance = { schema: "simurgh.varl.attester_provenance.v1", model_id: "m", org_id: "o", base_id: "synonym_veil_pct", response_digest: "sha256:0" }), c))),
    251
  );
  ...
  // 253 audit-only
  assert.equal(evaluateVarl(b, { tier: "audit", auditPrivate: { ...log, attempt_count: 0 } }).raw, 253);
  assert.equal(evaluateVarl(b, { tier: "public" }).raw, 0);
  // 254 wrapper
  assert.equal(evaluateVarlSafe(b, { tier: "audit", auditPrivate: { schema: "x", rounds: [], z: 1n } }).raw, 254);
});
...
test("read-only predecessor: 4w/4x/4y/5c byte-identical to the merge-base", () => {
  ...
  const changed = execFileSync(
    "git",
    ["diff", "--name-only", base, "--",
      "tools/simurgh-attestation/stage4w", "tools/simurgh-attestation/stage4x",
      "tools/simurgh-attestation/stage4y", "tools/simurgh-attestation/stage5c"],
