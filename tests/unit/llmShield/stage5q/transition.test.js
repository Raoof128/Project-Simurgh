...
test("a TREE-RELATIVE command is not_comparable — never 'pre-existing'", () => {
  // `--range HEAD~1..HEAD` names a different commit pair in every tree, so the baseline run
  // answers a different question. The first version labelled it `pre_existing`, which read as
  // "not our fault" about a check that had just caught a real Q0 violation: two Stage-4H evidence
  // files regenerated by check-e2e.sh and swept into a commit by `git add -A`.
  const a = attributeManifest(
    [{ command: "write surface HEAD~1..HEAD", ok: false, tree_relative: true }],
    [{ command: "write surface HEAD~1..HEAD", ok: false }]
  );
  assert.deepEqual(a.not_comparable, ["write surface HEAD~1..HEAD"]);
  assert.deepEqual(a.pre_existing, []);
  assert.deepEqual(a.regressed_by_q0, []);
});

test("not_comparable is reported in T7's detail, not swallowed", () => {
  const w = GREEN();
  w.manifestResults = [{ command: "wsr", ok: false, tree_relative: true }];
  w.baselineResults = [{ command: "wsr", ok: false }];
  const r = evaluateTransition(w);
  assert.equal(only(r, "T7").ok, false);
  assert.match(only(r, "T7").detail, /NOT COMPARABLE \(tree-relative\)/);
});

test("the manifest never runs in the primary worktree", () => {
  // check-e2e.sh regenerates Stage 4H's exit-map in whatever tree it runs in. The gate that
  // verifies non-disturbance must not be the thing that disturbs — same shape as 5Q-F003, one
  // layer up.
  const source = readFileSync(
    "tools/simurgh-attestation/stage5q/node/verifyTransition.mjs",
    "utf8"
  );
  assert.match(source, /runManifestIsolated\("HEAD", "HEAD"\)/);
  assert.equal(
    /const manifestResults = wantManifest \? runManifest\(\)/.test(source),
    false,
