...
All three daemons produce proofs that satisfy the same \texttt{simurgh-integrity-proof-v1} envelope. The canonical serialisation (\texttt{proofCanonicalise.js}) is mirrored in Swift (\texttt{ProofSigner.swift}) and Rust (\texttt{canonical\_json.rs}), with byte-identical golden fixture tests enforcing cross-implementation consistency. The server verifier applies the same E1 triple check and N1 node-continuity rule regardless of which daemon submitted the proof.
