...
operationalises the data-minimisation principle---collect only what the stated
purpose requires~\cite{klein2022pond,yeung2022demystifying}---as an enforced,
evidence-producing mechanism rather than a policy statement; no compliance
property is claimed or implied. \emph{Alternative considered:} schema-level
...
the explanation layer into a privacy sink. It therefore has four fail-closed
stages:
...
\textbf{LLM guardrails and AI risk frameworks.} Guardrail practice moderates live
model output with content filters, usage monitoring, and evaluation
frameworks~\cite{lee2024lifecycle}, and AI trust/risk/security frameworks
catalogue runtime enforcement and output filtering controls~\cite{ray2026trism}.
These filter content from a live model; \sys{} instead makes the provider
structurally unable to receive sensitive input, statically unable to egress, and
