...
#
# Fix 3: Stage 3K is a MEASUREMENT stage, not a defence change. This guard fails
...
BASE="${SIMURGH_STAGE3K_DIFF_BASE:-main}"

# Containment-policy files Stage 3K must not modify.
PROTECTED=(
...
for f in "${PROTECTED[@]}"; do
  if grep -qxF "$f" <<<"$changed"; then
    ...
    if [[ "$allowed" == "0" ]]; then
      echo "stage3k policy-drift FAIL: containment-policy file modified: $f"
      violation=1
    fi
  fi
done
...
if [[ "$violation" == "1" ]]; then
  echo "Stage 3K must not change defence logic. Add a justified ALLOWLIST entry if intentional."
  exit 1
fi
echo "stage3k policy-drift OK"
