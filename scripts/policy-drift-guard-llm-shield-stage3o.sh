...
# Stage 3O is a MEASUREMENT/tooling stage, not a defence change. Fails if the
# branch diff vs main touches any containment-policy source file.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
BASE="${SIMURGH_STAGE3O_DIFF_BASE:-main}"
PROTECTED=(
...
for f in "${PROTECTED[@]}"; do
  if grep -qxF "$f" <<<"$changed"; then
    ...
    if [[ "$allowed" == "0" ]]; then
      echo "stage3o policy-drift FAIL: containment-policy file modified: $f"
      violation=1
    fi
  fi
done
[[ "$violation" == "0" ]] && echo "stage3o policy-drift: passed" || exit 1
